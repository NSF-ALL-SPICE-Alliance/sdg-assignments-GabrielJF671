...
```{r}
goal_2_histogram <- ggplot(sdr_data, aes(x = goal_3_score, y = goal_4_score, color=regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Distributions of SDG 2 Scores",
       x = "SDG 2 Score",
       y = "Number of Countries",
       color = "Region")
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView(lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Reds", goal_3_score)(goal_5_score), label = mytext)
```
