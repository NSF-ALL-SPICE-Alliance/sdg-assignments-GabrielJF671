...
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_3_score, color = goal_9_score)) +
  geom_point()

# From this result, the Lowest Scores tend to become more Darker. Especially for certain Countries within those scores at between mid and heavier and lighter Darker of SDG 1 and 3. In addition, to SDG 9 being the lightest that represents those certain Countries.
```
...
#From this Chart, the countries that fall in the higher Category that are from the SDG 2 Score fall into LAC Region, while Countries are at the lower end of the Chart such as the Sub-Saharan Region.
```
